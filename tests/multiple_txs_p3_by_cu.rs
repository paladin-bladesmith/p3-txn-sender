...
// We transfer the same amount from tester 1 to 2, then from 2 to 3, and then from 3 back to 1.
...
async fn test_multiple_txs() {
    ...
    assert_eq!(
        before_balance_tester2 - result2.fee - transfer_amount,
        balance_tester2
    );
    assert_eq!(
    ...
    assert_eq!(
        before_balance_tester4 - result4.fee - transfer_amount,
        balance_tester4
    );
    assert_eq!(
